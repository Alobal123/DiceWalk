...
class GameObject(ABC):
    ...
    def draw_on_game(self, game) -> None:
        """Unified drawing entry point for game grid context.

        Default implementation defers to draw(); subclasses overriding draw_on_game
        can leverage additional game-specific context (e.g., isometric transforms).
        """
        try:
            self.draw()
        except Exception:
